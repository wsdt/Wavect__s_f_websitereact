...
function BreadCrumbs(props: IBreadCrumbsProps) {
    return <Breadcrumb style={{marginTop: 15}}>
        {props.breadcrumbs.map((breadcrumb: BreadcrumbConstants, index: number) => {
            ...
            if (index === props.breadcrumbs.length - 1) {
                // last iteration
                return <BreadcrumbItem key={breadcrumb.link} active={true} style={{fontWeight: 'bold'}} title={breadcrumb.title}>{shortenedTitle}</BreadcrumbItem>
            } else {
                return <BreadcrumbItem tag='a' href={breadcrumb.link} key={breadcrumb.link} title={breadcrumb.title}>{shortenedTitle}</BreadcrumbItem>
            }
        })
        }
    </Breadcrumb>
